...
import { MAIN_COVERAGE_DIR, RENDERER_COVERAGE_DIR } from './coverage-paths'
 ...
 *  - NODE_V8_COVERAGE       -> main-process V8 coverage dump
 * Renderer coverage is collected via the Playwright CDP coverage API.
 * Coverage is always on; the HTML report lands in coverage/ after every run.
 */
export const test = base.extend<Fixtures>({
  ...
  page: async ({ electronApp }, use, testInfo) => {
    ...
    writeFileSync(join(RENDERER_COVERAGE_DIR, `renderer-${safeId}.json`), JSON.stringify(entries))
  }
})
